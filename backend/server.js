...
const upload = multer({
    ...
    console.log('MIME type recibido:', file.mimetype);
    ...
    if (allowedMimes.includes(file.mimetype) || allowedExtensions.includes(ext)) {
        ...
        cb(new Error(`MIME type no permitido: ${file.mimetype}, Extensión: ${ext}. Solo se permiten CSV, Excel (.xlsx) o imágenes`), false);
    }
...
app.use((req, res, next) => {
    res.locals.cspNonce = crypto.randomBytes(16).toString('base64'); // Genera un nonce único
    next();
});
...
//app.use('/api', require('./routes/csv')); // Nueva ruta agregada
app.use('/api/csv', require('./routes/csv'));
app.use('/api/notifications', require('./routes/notifications')); // Asegúrate de que esta línea esté 
...
app.get('/api/dashboard', async (req, res) => {
    const token = req.headers.authorization?.split(' ')[1];
    if (!token) {
        logger.warn('Intento de acceso a /api/dashboard sin token');
    ...
    try {
        const user = verifyToken(token);
        if (user.role !== 'admin') {
            logger.warn(`Acceso denegado a /api/dashboard para usuario con rol: ${user.role}`);
            return res.status(403).json({ error: 'Solo administradores' });
        }

        // Ejemplo de datos del dashboard (ajusta según tus necesidades)
        const dashboardData = {
            totalSales: 15000, // Ejemplo, obtén datos reales de la base de datos
            totalOrders: 120,
            recentProducts: 5,
            pendingCSVs: await pool.query('SELECT COUNT(*) FROM pending_csvs WHERE status = $1', ['pending']).then(res => parseInt(res.rows[0].count))
        };

        logger.info(`Dashboard cargado para usuario ${user.id}`);
        res.json(dashboardData);
    } catch (error) {
        logger.error(`Error en /api/dashboard: ${error.message}`);
