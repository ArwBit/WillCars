...
function createWindow() {
    ...
    mainWindow.loadFile(path.join(__dirname, 'csvs.html'));
}
...
ipcMain.on('process-csv', async (event, { csvPath, proveedorId }) => {
    try {
        const records = await processCsv(csvPath, proveedorId);
        const outputPath = path.join(BASE_UPLOADS, proveedor.carpeta, 'productos_listos.csv');
        const csvWriter = createCsvWriter({
            path: outputPath,
            header: [
                { id: 'Código', title: 'Código' },
        ...
        event.reply('csv-processed', { success: true, message: `CSV procesado: ${records.length} registros` });
    } catch (err) {
        event.reply('csv-processed', { success: false, message: `Error: ${err.message}` });
    }
});

ipcMain.on('combine-csvs', async (event) => {
    try {
        const count = await combineCsvs();
        event.reply('csv-combined', { success: true, message: `CSV global generado con ${count} registros` });
    } catch (err) {
        event.reply('csv-combined', { success: false, message: `Error: ${err.message}` });
    }
});